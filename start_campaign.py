...
def get_ip():
    ...
    while True:
        try:
            CC = console.input('\n[cyan][-] Enter a DN/IP:Port for this campaign:[/cyan] ')
            CC = CC.split(':')
            IP = CC[0]
            PORT = CC[1]
            break
...
def update_template():
    global beacon, IP, PORT, date, Donut, url
    url = ''
    ...
    data = data.replace('{IP}', IP).replace('{beacon_time}', beacon).replace('{PORT}', PORT).replace('{URL}',
                                                                                                     url).replace(
        '{SSL}', SSL).replace('{CERT}', cert).replace('{KEY}', key).replace('{KDATE}', date).replace('{DONUT}', Donut)
    config.write(data)
    config.close()
    template.close()
    console.print("[++] Everything Done you can run ninja by : python3 Ninja.py", style="magenta")
...
def log_campaign():
    try:
        ...
        pass
    global beacon, IP, PORT, date, url
...
def kill_date():
    ...
    CC = ''
    while len(CC) == 0:
        CC = console.input('[cyan][-] Please enter kill date for this campaign ([green]format dd/MM/yyyy[/green]): [/cyan]')
        if len(CC.split("/")) > 2:
            try:
