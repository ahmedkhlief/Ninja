...
def PCinfo(pcinfo):
    ...
    print(
        "PC Report : \n  Host Name: %s \n  OS : %s \n  Build Number : %s \n  Local Time : %s \n  Time Zone : %s \n  Last Boot Time : %s \n " % (
        pclist["CSName"], pclist["Caption"], pclist["BuildNumber"], pclist["LocalDateTime"], pclist["CurrentTimeZone"],
        pclist["LastBootUpTime"]), "Bios Manufacturer : " + pclist["Manufacturer"].strip() + " , " + pclist[
            "SMBIOSBIOSVersion"].strip() + " \n###################################")

    if pclist["Manufacturer"].strip().lower().find("innotek") > -1 or pclist["SMBIOSBIOSVersion"].strip().lower().find(
            "virtualbox") > -1:
        sandbox.append(9)
...
def getscore():
    ...
    avg = (sum / len(score))
    if avg <= 4:
        print(bcolors.OKGREEN + "Hardness score (" + str(
            avg) + "/10" + ") : Easy , you can pwn the system easily" + bcolors.ENDC)
    if avg >= 5 and avg <= 7:
        print(bcolors.WARNING + "Hardness score (" + str(
            avg) + "/10" + ") : Medium , you can pwn the system with good enumeration and available local exploit " + bcolors.ENDC)
    if avg > 7:
        print(bcolors.FAIL + "Hardness score (" + str(
            avg) + "/10" + ") : Hard , be careful from the AV and the security updates installed" + bcolors.ENDC)
    sum = 0
    for i in sandbox:
        # print i,
        sum = sum + i
    avg = (sum / len(sandbox))
    if avg <= 4:
        print(bcolors.OKGREEN + "Sandbox Score (" + str(
            avg) + "/10" + ") : you are probably in real live system" + bcolors.ENDC)
    if avg >= 5 and avg <= 7:
        print(bcolors.WARNING + "Sandbox Score (" + str(
            avg) + "/10" + ") : check and confirm as you probably in a security analyst device" + bcolors.ENDC)
    if avg > 7:
        print(bcolors.FAIL + "Sandbox Score (" + str(avg) + "/10" + ") : you are in a sandbox" + bcolors.ENDC)
    print("###################################")


def main(fname=f"{config.campaign_name}/DA/DA_out.txt"):
    try:
        file = open(fname)
        data = file.read()
        data = data.split("###############")
